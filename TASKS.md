...
| 1.1 | Define the persona frontmatter schema (see AGENTS.md §Persona schema), then create four starter persona files: `sa.md`, `sre.md`, `finops.md`, `security.md` — sa/sre/finops as `role_type: debater`, security as `role_type: guardian` with `compliance_ref: "backend/personas/compliance/*.md"`. Files must include all required frontmatter fields. | Phase 1 | 0.3 | Done |
| 1.2 | Create `backend/personas/compliance/` directory with at least two BYOC mandate files (`iam.md`, `data-residency.md`) using the mandate template from `gitops-persona-skill.md` | Phase 1 | 1.1 | Done |
...
| 2.4r | Build persona loader (`lib/debate/load-personas.ts`): reads all `*.md` files in `backend/personas/agents/`, parses YAML frontmatter, validates required fields, filters `enabled: true`, sorts by `turn_order`, enforces max-6-debater cap (warns and truncates, does not throw), returns `PersonaConfig[]` typed array | Phase 2 | 1.1 | Done |
...
| 2.7 | Implement synthesis step (`lib/debate/synthesis.ts`): single call to `meta-llama/llama-3-3-70b-instruct` outside the loop, converts final state to canonical architecture description; flag unresolved objections in output | Phase 2 | 2.6 | Done |
...
| 5.2 | Add a cached-response fallback for the synthesis step: if `meta-llama/llama-3-3-70b-instruct` returns an error, use the last round's SA proposal as the synthesis output rather than failing silently | Phase 5 | 5.1 | Done |
